...
const STEPS = ['Details', 'Source', 'Destination', 'Tables', 'Options'];
  ...
  useEffect(() => {
    ...
    socket.on('progress', ({ jobId, progress: p, tableName, migratedRecords, totalRecords }) => {
      ...
      setMigrations((prev) =>
        prev.map((m) =>
          m.id === jobId
            ? { ...m, progress: p, migrated_records: migratedRecords, total_records: totalRecords }
            : m
        )
      );
      addLog(
        jobId,
        `[PROGRESS] ${tableName}: ${migratedRecords?.toLocaleString()}/${totalRecords?.toLocaleString()} (${p?.toFixed(1)}%)`
      );
    });
    socket.on('table_complete', ({ jobId, tableName, tableMigrated }) => {
      addLog(
        jobId,
        `[SUCCESS] Table "${tableName}" completed: ${tableMigrated?.toLocaleString()} records`
      );
    });
    ...
    socket.on('job_status', ({ jobId, status, migratedRecords, totalRecords }) => {
      setMigrations((prev) =>
        prev.map((m) =>
          m.id === jobId
            ? { ...m, status, migrated_records: migratedRecords || m.migrated_records }
            : m
        )
      );
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            {migrations.filter((m) => m.status === 'running').length} running
          </p>
        </div>
        <button
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={9} className="text-center py-12 text-slate-400">
                    No migrations yet. Start your first migration!
                  </td>
                </tr>
              ) : (
                migrations.map((m) => (
                  <tr key={m.id} className="cursor-pointer" onClick={() => setSelectedJob(m.id)}>
                    ...
                    <td className="text-xs">
                      <span className="text-slate-600 dark:text-slate-300">
                        {m.sourceConnection?.name || '—'}
                      </span>
                      <span className="text-slate-400 mx-1">→</span>
                      <span className="text-slate-600 dark:text-slate-300">
                        {m.destinationConnection?.name || '—'}
                      </span>
                    </td>
                    <td>
                      <span className="badge-neutral">{m.tables?.length ?? 0}</span>
                    </td>
                    <td>
                      <StatusBadge status={m.migration_type} />
                    </td>
                    <td>
                    ...
                    <td className="min-w-[120px]">
                      <ProgressBar
                        value={progress[m.id] ?? m.progress ?? 0}
                        showPercent={false}
                        height="h-1.5"
                      />
                      <span className="text-xs text-slate-400">
                        {(progress[m.id] ?? m.progress ?? 0).toFixed(1)}%
                      </span>
                    </td>
                    <td className="text-xs text-slate-500">
                      {m.migrated_records?.toLocaleString() ?? 0}/
                      ...
                      <div className="flex gap-1">
                        ...
                        {m.status === 'failed' && (
                          <button
                            ...
                            className="btn-ghost text-xs py-1 px-2 text-primary-500"
                          >
                            ↺
                          </button>
                        )}
        ...
        <div className="card p-5 animate-fade-in">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">
              Live Logs — {selectedJob.slice(0, 8)}…
            </p>
            <button onClick={() => setSelectedJob(null)} className="btn-ghost text-xs">
              ✕ Close
            </button>
          </div>
          <div className="log-block">
            {liveLogs[selectedJob].map((l, i) => (
              <div
                key={i}
                className={`mb-0.5 ${l.includes('ERROR') ? 'text-rose-400' : l.includes('SUCCESS') ? 'text-emerald-400' : l.includes('PROGRESS') ? 'text-cyan-400' : 'text-slate-300'}`}
              >
                {l}
              </div>
            ))}
      ...
      >
        ...
        <div className="flex items-center gap-1 mb-6">
          {STEPS.map((s, i) => (
            <React.Fragment key={s}>
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={`flex-1 h-px ${i < step ? 'bg-emerald-400' : 'bg-slate-200 dark:bg-slate-600'}`}
                />
              )}
            </React.Fragment>
          ))}
        ...
        {step === 0 && (
          <div className="space-y-4">
            <div>
              ...
              <input
                className="input"
                placeholder="e.g. Prod to Staging Migration"
                value={form.name}
        ...
        {step === 1 && (
          <div className="space-y-3">
            <p className="text-sm text-slate-500 mb-2">
              Select the source database to migrate FROM
            </p>
            {connections.length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-500 p-6 text-center">
                <p className="font-medium text-slate-700 dark:text-slate-200">
                  No database connections configured
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  Add and test at least two connections: one source and one destination.
                </p>
                <button type="button" className="btn-primary mt-4" onClick={openConnections}>
              ...
              connections.map((c) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {form.source_connection_id === c.id && (
        ...
        {step === 2 && (
          <div className="space-y-3">
            <p className="text-sm text-slate-500 mb-2">
              Select the destination database to migrate TO
            </p>
            {connections.filter((c) => c.id !== form.source_connection_id).length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-500 p-6 text-center">
                <p className="font-medium text-slate-700 dark:text-slate-200">
                  A second connection is required
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  The destination must be different from the selected source.
                </p>
                <button type="button" className="btn-primary mt-4" onClick={openConnections}>
              ...
              connections
                .filter((c) => c.id !== form.source_connection_id)
                .map((c) => (
                <div
                  key={c.id}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {form.destination_connection_id === c.id && (
                      <span className="ml-auto text-primary-500">✓</span>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm text-slate-500">
                Select tables to migrate ({form.tables.length} selected)
              </p>
              <button
                ...
                onClick={() =>
                  setForm((f) => ({
                    ...f,
                    tables: f.tables.length === availableTables.length ? [] : [...availableTables],
                  }))
                }
              >
            ...
            {loadingTables ? (
              ...
              <div className="text-center text-slate-400 py-6">
                No tables found. Check your connection settings.
              </div>
            ) : (
        ...
        {step === 4 && (
          <div className="space-y-4">
            <div>
              ...
              <div className="p-3 rounded-xl border border-primary-500 bg-primary-50 dark:bg-primary-900/20">
                <p className="font-medium text-sm text-slate-800 dark:text-white">Full Migration</p>
                <p className="text-xs text-slate-400 mt-1">
                  Copies all selected rows. Incremental migration requires a configured checkpoint
                  key, so it is intentionally disabled for now.
                </p>
              </div>
            </div>
            <div>
              <label className="input-label">When destination already has data</label>
              <select
                className="input"
                value={form.options.existing_data_strategy}
                onChange={(e) =>
                  setForm({
                    ...form,
                    options: { ...form.options, existing_data_strategy: e.target.value },
                  })
                }
              >
                <option value="fail">Stop and protect existing data</option>
                <option value="skip">Skip rows that conflict with a key</option>
                <option value="truncate">Delete destination rows, then migrate</option>
              </select>
            </div>
            <label className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
              <input
                type="checkbox"
                checked={form.options.create_missing_tables}
                onChange={(e) =>
                  setForm({
                    ...form,
                    options: { ...form.options, create_missing_tables: e.target.checked },
                  })
                }
              />
              Create missing destination tables automatically
            </label>
            <label className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
              <input
                type="checkbox"
                checked={form.options.continue_on_error}
                onChange={(e) =>
                  setForm({
                    ...form,
                    options: { ...form.options, continue_on_error: e.target.checked },
                  })
                }
              />
              Continue with other tables when one table fails
            </label>
            <div>
