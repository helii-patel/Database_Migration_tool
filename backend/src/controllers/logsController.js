...
const getLogs = async (req, res, next) => {
  try {
    ...
    if (search) where[Op.or] = [{ action: { [Op.like]: `%${search}%` } }, { username: { [Op.like]: `%${search}%` } }];
    if (action) where.action = action;
    if (user_id) where.user_id = user_id;
    if (status) where.status = status;
    if (startDate && endDate) where.created_at = { [Op.between]: [new Date(startDate), new Date(endDate)] };

    const offset = (parseInt(page) - 1) * parseInt(limit);
    const { count, rows } = await AuditLog.findAndCountAll({
      where, order: [['created_at', 'DESC']], limit: parseInt(limit), offset,
    });
    res.json({ success: true, data: { logs: rows, total: count, page: parseInt(page), totalPages: Math.ceil(count / parseInt(limit)) } });
  } catch (err) { next(err); }
};
