...
const { createConnection, getTables, getTableSchema, closeConnection } = require('../services/dbAdapter');
const logger = require('../utils/logger');

const getAll = async (req, res, next) => {
  try {
    const where = req.user.role === 'admin' ? {} : { created_by: req.user.id };
    const connections = await DatabaseConnection.findAll({ where, order: [['created_at', 'DESC']] });
...
const create = async (req, res, next) => {
  try {
    const { name, db_type, host, port, database_name, username, password, ssl_enabled, description, tags } = req.body;
    const encrypted_password = encrypt(password);
    const conn = await DatabaseConnection.create({
      name, db_type, host, port, database_name, username, encrypted_password,
      ssl_enabled: !!ssl_enabled, description, tags, created_by: req.user.id,
    });
    await AuditLog.create({ user_id: req.user.id, username: req.user.username, action: 'CONNECTION_CREATED', resource_type: 'DatabaseConnection', resource_id: conn.id, ip_address: req.ip });
    const safe = conn.toJSON(); delete safe.encrypted_password;
...
const update = async (req, res, next) => {
  try {
    ...
    if (conn.created_by !== req.user.id && req.user.role !== 'admin')
...
const remove = async (req, res, next) => {
  try {
    ...
    if (conn.created_by !== req.user.id && req.user.role !== 'admin')
      return res.status(403).json({ success: false, message: 'Not authorized' });
    await conn.destroy();
    await AuditLog.create({ user_id: req.user.id, username: req.user.username, action: 'CONNECTION_DELETED', resource_type: 'DatabaseConnection', resource_id: req.params.id, ip_address: req.ip });
...
const testConnection = async (req, res, next) => {
  try {
    ...
    try {
      ...
      res.json({ success: true, message: 'Connection successful', data: { table_count: tables.length } });
    } catch (err) {
