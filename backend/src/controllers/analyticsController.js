const { MigrationJob, DatabaseConnection, ValidationReport, PerformanceSnapshot, AuditLog } = require('../models');
...
const getOverview = async (req, res, next) => {
  try {
    ...
    const [
      totalMigrations, completedMigrations, failedMigrations, runningMigrations,
      totalConnections, totalValidations, passedValidations,
    ] = await Promise.all([
    ...
    const validationRate = totalValidations > 0 ? Math.round((passedValidations / totalValidations) * 100) : 0;

    // Get total records migrated
    const totalRecordsMigrated = await MigrationJob.sum('migrated_records', { where: { ...where, status: 'completed' } }) || 0;
    ...
    recentJobs.forEach((j) => {
      const key = j.created_at.toISOString().split('T')[0];
      if (trendMap[key]) {
        trendMap[key].total++;
    ...
    const healthScore = Math.max(0, Math.min(100,
      60 + (successRate * 0.25) + (validationRate * 0.15) - (failedMigrations * 2)
    ));
    ...
    res.json({
      ...
      data: {
        kpis: {
          totalMigrations, completedMigrations, failedMigrations, runningMigrations,
          successRate, totalConnections, totalRecordsMigrated,
          validationRate, healthScore: Math.round(healthScore),
        },
